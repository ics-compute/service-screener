...
class OutputGenerator:
    ...
    def generate(self, contexts, regions, frameworks=None):
        ...
        # If beta mode, also generate Cloudscape. The legacy report remains the
        # resilient landing page if a local Node build or data embedding fails.
        if self.beta_mode:
            ...
            try:
                ...
                if success:
                    ...
                    self._restore_legacy_index()
                    _warn("Cloudscape build failed. Restored AdminLTE as index.html.")
            except Exception as e:
                self._restore_legacy_index()
                _warn(f"Cloudscape generation failed: {e}. Restored AdminLTE as index.html.")

    def _restore_legacy_index(self):
        """Restore the always-generated AdminLTE landing page after beta failure."""
        legacy_index = os.path.join(self.html_folder, 'index-legacy.html')
        current_index = os.path.join(self.html_folder, 'index.html')
        if os.path.exists(legacy_index):
            shutil.copy2(legacy_index, current_index)
            _info("Legacy AdminLTE index restored after Cloudscape fallback")
